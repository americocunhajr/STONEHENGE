...
% -----------------------------------------------------------------
function plot_piezomagbeam_animation_inertial_asymmetric(series, f  ,Omega,...
...
warning('off')
    ...
    % loop to construct the video
     for n=1:Njump:Ndt 
        ...
        % cossine of Omega t
		cos_Omega_t = f*cos(Omega*time(n));
        ...
        fh13 = rectangle('Position' ,[xv1_0-0.2,-1.11,1.45,0.01], ...
        ...
        % magnets coordinates
        xm1 = xv1+0.25*rb_length-0.5;
        ym1 = yv6;
        Axm1 = [xm1 xm1 xm1+0.10*rb_length xm1+0.10*rb_length];
        Aym1 = [ym1 ym1+0.10*rb_length ym1+0.10*rb_length ym1];
        Am1 = [Axm1;Aym1];
        mag1_rot = T*Am1;
        
        fh03 = fill(mag1_rot(1,:),mag1_rot(2,:),[0.91 0.41 0.17], ...
                                    'EdgeColor',[0.91 0.41 0.17]);
        
        xm2 = xv1 + 0.75*rb_length-0.5;
        ym2 = yv6;
        Axm2 = [xm2 xm2 xm2+0.10*rb_length xm2+0.10*rb_length];
        Aym2 = [ym2 ym2+0.10*rb_length ym2+0.10*rb_length ym2];
        Am2 = [Axm2;Aym2];
        mag1_rot = T*Am2;
        
        fh04 = fill(mag1_rot(1,:),mag1_rot(2,:),[0.91 0.41 0.17], ...
                                    'EdgeColor',[0.91 0.41 0.17]);
        
		
	    % resistor coordinates
    	xr1 = xvline1 - 0.1*rb_length+0.08-0.08;
	    yr1 = 1.2*yv2;
    	xc2 = xvline1 - 0.15*rb_length+0.08-0.08;
	    yc2 = 0.7*yv4;
        
        Axc = [xc2 xc2 xc2+0.3*rb_length xc2+0.3*rb_length xc2];
        Ayc = [yc2 yc2+0.40*rb_length yc2+0.40*rb_length yc2 yc2];
        Ac = [Axc; Ayc];
        circ_rot = T*Ac;
        
        % draw circuit
        fh05 = plot(circ_rot(1,:),circ_rot(2,:),'Color',[0 0 0]);
		
        Axr = [xr1 xr1 xr1+0.2*rb_length xr1+0.2*rb_length xr1];
        Ayr = [yr1 yr1+0.05*rb_length yr1+0.05*rb_length yr1 yr1];
        Ar = [Axr; Ayr];
        rest_rot = T*Ar;
        
        % draw resistor
        fh06 = fill(rest_rot(1,:),rest_rot(2,:),[0 1 0]);        
        ...
        if beta ~= 0
            ...
            Cnc = T*[0.7-cos_Omega_t*0.17 0.77-cos_Omega_t*0.17; 0.65 0.68];
        ...
        text(-0.6*xmin,0.4*ymin,['  inertial ', newline , ...
                                 '  frame of ' , newline , ...
                                 ' reference']);
